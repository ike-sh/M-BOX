...
const SECTIONS: { id: SectionId; label: string; icon: typeof Cpu }[] = [
  { id: "ports", label: "端口设置", icon: Network },
  { id: "auth", label: "代理认证", icon: KeyRound },
  { id: "basic", label: "基础设置", icon: SlidersHorizontal },
  { id: "perf", label: "性能优化", icon: Zap },
  { id: "network", label: "网络设置", icon: Wifi },
  { id: "dns", label: "DNS 设置", icon: Waypoints },
  { id: "tun", label: "TUN 设置", icon: Shield },
  { id: "sniffer", label: "流量嗅探", icon: Radar },
  { id: "geo", label: "GEO 数据", icon: Database },
];
    ...
    <div className="page">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 16, alignItems: "start" }}>
        ...
        <div className="col" style={{ gap: 16, minWidth: 0 }}>
          {active === "dns" && <Dns />}
          {active === "tun" && <Tun />}
          {needGen && !gen && <GlassCard><span className="muted-2" style={{ fontSize: 13 }}>加载中…</span></GlassCard>}
      ...
      {resetOpen && (
        <ConfirmDialog
          title="恢复默认配置？"
          message={<>将把 config.yaml 重置为内置默认配置（会<b>自动备份</b>当前配置、保留控制器密钥）。<b>你的订阅注入、自定义节点、DNS / 分流 / 各项设置改动都会被清空</b>，确定继续？</>}
          confirmText="重置为默认"
          danger
          busy={resetting}
          onConfirm={async () => {
            ...
            try {
              ...
              setToast({ kind: "ok", text: "已恢复默认并热重载" });
              api.getGeneral().then(setGen);
            } catch (e) {
              setToast({ kind: "err", text: `重置失败：${e instanceof Error ? e.message : "请重试"}` });
            } finally {
              setResetting(false);
              setResetOpen(false);
            }
...
function PortsSettings({ gen, patch }: SectionProps) {
  return (
    <GlassCard>
      <CardHead icon={<Network size={18} color="var(--blue)" />} title="端口设置" sub="本地代理与透明代理监听端口（开关启用，0 = 禁用，仅绑本机）" />
      <div className="col">
        <PortRow label="混合代理端口" desc="HTTP/SOCKS5 复用端口" value={gen.mixedPort} def={7890} onChange={(v) => patch({ mixedPort: v })} />
        <PortRow label="SOCKS5 端口" desc="独立 SOCKS5 代理端口" value={gen.socksPort} def={7891} onChange={(v) => patch({ socksPort: v })} />
        <PortRow label="HTTP 端口" desc="独立 HTTP 代理端口" value={gen.httpPort} def={7892} onChange={(v) => patch({ httpPort: v })} />
      </div>
    </GlassCard>
  );
}
    ...
    <div className="kv" style={{ gap: 12 }}>
      <div className="row" style={{ gap: 12, minWidth: 0, alignItems: "center" }}>
        ...
        <div className="col" style={{ minWidth: 0 }}>
          ...
          <span className="muted-2" style={{ fontSize: 11.5 }}>{desc}（0=禁用）</span>
        </div>
      </div>
      <input
...
function BasicSettings({ gen, patch }: SectionProps) {
  ...
  return (
    <GlassCard>
      <CardHead icon={<SlidersHorizontal size={18} color="var(--blue)" />} title="基础设置" sub="运行模式、局域网、日志、IPv6 等基础参数" />
      <div className="col">
        <Row label="允许局域网连接" desc="允许其他设备通过本机端口代理上网">
          <Switch on={gen.allowLan} onChange={(v) => patch({ allowLan: v })} />
        </Row>
        <Row label="运行模式" desc="rule 按规则 / global 全局 / direct 直连">
          <Segmented value={mode} onChange={(v) => { setMode(v); api.setMode(v); }} options={[{ value: "rule", label: "规则" }, { value: "global", label: "全局" }, { value: "direct", label: "直连" }]} />
        </Row>
        <Row label="日志级别" desc="排查问题时可调 debug">
          <Segmented value={gen.logLevel} onChange={(v) => patch({ logLevel: v })} options={[{ value: "silent", label: "静默" }, { value: "warning", label: "警告" }, { value: "info", label: "信息" }, { value: "debug", label: "调试" }]} />
        </Row>
        <Row label="启用 IPv6" desc="一处协调顶层 ipv6 与 dns.ipv6，避免半开泄漏">
          {ipv6 ? <Switch on={ipv6.enabled} onChange={(v) => { setIpv6((s) => (s ? { ...s, enabled: v, top: v, dns: v, consistent: true } : s)); api.applyIPv6(v).then(setIpv6); }} /> : <span className="muted-2" style={{ fontSize: 12 }}>加载中…</span>}
        </Row>
      </div>
    </GlassCard>
  );
...
function PerfSettings({ gen, patch }: SectionProps) {
  return (
    <GlassCard>
      <CardHead icon={<Zap size={18} color="var(--purple)" />} title="性能优化" sub="延迟测算、并发、进程匹配、TLS 指纹等高级项" />
      <div className="col">
        <Row label="统一延迟计算" desc="更准确的延迟测试结果">
          <Switch on={gen.unifiedDelay} onChange={(v) => patch({ unifiedDelay: v })} />
        </Row>
        <Row label="TCP 并发连接" desc="使用所有解析 IP 同时连接，取最快">
          <Switch on={gen.tcpConcurrent} onChange={(v) => patch({ tcpConcurrent: v })} />
        </Row>
        <Row label="进程匹配模式" desc="网关模式建议关闭">
          <Segmented value={gen.findProcessMode || "off"} onChange={(v) => patch({ findProcessMode: v })} options={[{ value: "off", label: "关闭" }, { value: "strict", label: "严格" }, { value: "always", label: "总是" }]} />
        </Row>
        <Row label="TLS 客户端指纹" desc="伪装 TLS 指纹">
          <Segmented value={gen.globalClientFingerprint || ""} onChange={(v) => patch({ globalClientFingerprint: v })} options={[{ value: "", label: "关闭" }, { value: "chrome", label: "Chrome" }, { value: "firefox", label: "Firefox" }, { value: "safari", label: "Safari" }, { value: "random", label: "随机" }]} />
        </Row>
      </div>
    </GlassCard>
  );
...
function NetworkSettings({ gen, patch }: SectionProps) {
  return (
    <GlassCard>
      <CardHead icon={<Wifi size={18} color="var(--green)" />} title="网络设置" sub="出站网卡、路由标记、Keep-Alive 等网络相关参数" />
      <div className="col">
        <TextRow label="出站网卡" desc="指定出站网络接口名称（留空自动检测）" value={gen.interfaceName} placeholder="eth0" onCommit={(v) => patch({ interfaceName: v })} />
        <NumRow label="路由标记 (fwmark)" desc="Linux 路由标记，0 = 不设置" value={gen.routingMark} onCommit={(v) => patch({ routingMark: v })} />
        <NumRow label="Keep-Alive 间隔(秒)" desc="TCP 保活探测间隔" value={gen.keepAliveInterval} onCommit={(v) => patch({ keepAliveInterval: v })} />
        <NumRow label="Keep-Alive 空闲(秒)" desc="TCP 空闲多久后开始保活" value={gen.keepAliveIdle} onCommit={(v) => patch({ keepAliveIdle: v })} />
        <Row label="禁用 Keep-Alive" desc="某些环境断连频繁时可尝试开启">
          <Switch on={gen.disableKeepAlive} onChange={(v) => patch({ disableKeepAlive: v })} />
        </Row>
        <div style={{ margin: "10px 0 2px", fontSize: 12.5, fontWeight: 600, color: "var(--t2)" }}>延迟测试设置（用于「节点管理」的节点测速）</div>
        <TestUrlRow value={gen.testUrl} onCommit={(v) => patch({ testUrl: v })} />
        <NumRow label="测速超时(ms)" desc="单节点延迟测试超时时间" value={gen.testTimeout} onCommit={(v) => patch({ testTimeout: v })} />
        <NumRow label="测速间隔(秒)" desc="自动测速的时间间隔（0 = 不自动）" value={gen.testInterval} onCommit={(v) => patch({ testInterval: v })} />
      </div>
    </GlassCard>
  );
}
...
function TestUrlRow({ value, onCommit }: { value: string; onCommit: (v: string) => void }) {
  ...
  return (
    <Row label="测速 URL" desc="用于测试节点延迟的 URL 地址">
...
function SnifferSettings({ gen, patch }: SectionProps) {
  ...
  return (
    <GlassCard>
      <CardHead icon={<Radar size={18} color="var(--blue)" />} title="流量嗅探" sub="从 HTTP/TLS/QUIC 握手提取真实域名，提升按域名分流准确性" />
      <div className="col">
        <Row label="启用嗅探"><Switch on={sn.enable} onChange={(v) => setSn({ enable: v })} /></Row>
        <Row label="覆盖目标地址" desc="用嗅探域名重置请求目标（更激进，默认关更稳）"><Switch on={sn.overrideDestination} onChange={(v) => setSn({ overrideDestination: v })} /></Row>
        <Row label="嗅探 HTTP"><Switch on={sn.http} onChange={(v) => setSn({ http: v })} /></Row>
        <Row label="嗅探 TLS"><Switch on={sn.tls} onChange={(v) => setSn({ tls: v })} /></Row>
        <Row label="嗅探 QUIC"><Switch on={sn.quic} onChange={(v) => setSn({ quic: v })} /></Row>
      </div>
    </GlassCard>
  );
}
...
function AuthSettings({ gen, patch }: SectionProps) {
  ...
  return (
    <GlassCard>
      <CardHead icon={<KeyRound size={18} color="var(--orange)" />} title="代理认证" sub="为本地代理端口设置用户名/密码（透明代理走 TUN 不受影响）" />
      <div className="col gap-2">
        <div className="row gap-2" style={{ alignItems: "stretch" }}>
          <input className="input" style={{ flex: 1, fontSize: 12.5 }} placeholder="用户名" value={user} onChange={(e) => setUser(e.target.value)} />
          <input className="input" style={{ flex: 1, fontSize: 12.5 }} placeholder="密码" value={pass} onChange={(e) => setPass(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") add(); }} />
          <button className="btn btn-primary" onClick={add} disabled={!user.trim() || !pass.trim()}><Plus size={15} /> 添加用户</button>
        </div>
        {gen.authentication.length === 0 ? (
          <span className="muted-2" style={{ fontSize: 12 }}>（未设置认证，本地代理端口免密；公网暴露时务必设置）</span>
        ) : (
          gen.authentication.map((a) => {
            ...
            return (
              <div key={a} className="row between" style={{ padding: "8px 12px", borderRadius: "var(--r-sm)", background: "var(--fill-2)", gap: 8 }}>
                ...
                <button className="icon-btn" style={{ width: 26, height: 26 }} title="删除" onClick={() => remove(a)}><X size={13} /></button>
              </div>
            );
          })
        )}
...
function GeoSettings({ gen, patch }: SectionProps) {
  ...
  async function update() {
    ...
    try { await api.updateGeo(); setToast("GEO 数据已更新并热加载"); }
    catch { setToast("更新失败：内核未运行或下载超时"); }
    finally { setBusy(false); setTimeout(() => setToast(null), 3000); }
  }
  return (
    <GlassCard>
      <CardHead icon={<Database size={18} color="var(--teal)" />} title="GEO 数据" sub="GeoIP / GeoSite 数据库：地理位置判断与域名分类" />
      <div className="col">
        <Row label="使用 DAT 格式" desc="DAT 格式查询更快">
          <Switch on={gen.geodataMode} onChange={(v) => patch({ geodataMode: v })} />
        </Row>
        <Row label="加载器" desc="standard 兼容 / memconservative 省内存">
          <Segmented value={gen.geodataLoader || "memconservative"} onChange={(v) => patch({ geodataLoader: v })} options={[{ value: "standard", label: "标准" }, { value: "memconservative", label: "省内存" }]} />
        </Row>
        <Row label="自动更新" desc="后台定时更新 GEO 数据库">
          <Switch on={gen.geoAutoUpdate} onChange={(v) => patch({ geoAutoUpdate: v })} />
        </Row>
        <NumRow label="更新间隔(小时)" desc="自动更新的时间间隔" value={gen.geoUpdateInterval} onCommit={(v) => patch({ geoUpdateInterval: v })} />
        <TextRow label="下载 UA" desc="下载外部资源时使用的 User-Agent" value={gen.globalUa} placeholder="clash.meta" onCommit={(v) => patch({ globalUa: v })} />
        <div className="row gap-2" style={{ alignItems: "center", marginTop: 6 }}>
          <button className="btn btn-primary" onClick={update} disabled={busy}><Database size={15} /> {busy ? "更新中…" : "立即更新 GEO 数据"}</button>
